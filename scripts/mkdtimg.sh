...
TOOL_DTC_FOLDER=$(pwd)/out/scripts/dtc
...
for dts_file in ${dts_array[@]}; do
	if [ -e $dts_file ]; then
		$TOOL_DTC -I dts -O dtb $dts_file -o $(pwd)/out/arch/arm/boot/dts/$dts_file.dtb
	fi
done

$TOOL_DTBTOOL -o $dtimg $(pwd)/out/arch/arm/boot/dts/ -p $TOOL_DTC_FOLDER/
